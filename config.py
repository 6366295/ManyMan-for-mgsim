...
See U{this tutorial<http://www.red-dove.com/python_config.html|_blank>} for
more information.
...
class Config(Mapping):
    ...
    def __init__(self, streamOrFile=None, parent=None):
        ...
        if streamOrFile is not None:
            if isinstance(streamOrFile, StringType) or \
                isinstance(streamOrFile, UnicodeType):
                global streamOpener
...
class Sequence(Container):
    ...
    def __str__(self):
        # using the slice evaluates the contents
        return str(self[:])
        ...
        while current is not None:
            if self.type == BACKTICK:
                ...
                for ns in namespaces:
                    try:
                        try:
                            ...
                            #Python 2.7 - vars is a dictproxy
                            rv = eval(s, {}, vars(ns))
                        found = True
                        break
                    except:
                ...
                try:
                    ...
                    logger.debug(
                        "Unable to resolve %r: %s", key, sys.exc_info()[1]
                    )
                    rv = None
                    pass
                current.resolving.discard(firstkey)
            current = self.findConfig(
                object.__getattribute__(current, 'parent')
            )
        if current is None:
            raise ConfigResolutionError(
...
class Expression(object):
    """
    This internal class implements a value which is obtained by evaluating an
    expression.
    """
            ...
            if c in self.quotes:
                ...
                while True:
                    ...
                    if (c == quote) and not escaped:
                        if not multiline or \
                            (len(token) >= 6 and token.endswith(token[:3]) \
                            and token[-4] != '\\'):
                            break
                    if c == '\\':
                        escaped = not escaped
    ...
    def parseSuffix(self, ref):
        ...
        if tt == DOT:
            ...
            if tt not in [NUMBER, STRING]:
                raise ConfigFormatError(
                    "%s: expected number or string, found %r" % \
                    (self.location(), tv)
                )
            self.token = self.getToken()
            tv = eval(tv)
